...
top_director_3 as
(
select nm.name as director_name,count(g.movie_id) as movie_count,
...
select distinct name actor_name, count(r.movie_id) as movie_count
from ratings as r
inner join 
role_mapping as rm
...
dense_rank() over (order by sum(total_votes) desc) as prod_com_rank
from movie as m
inner join
ratings as r
on m.id= r.movie_id
...
select title, 
case
when avg_rating > 8 then 'superhit_movie'
when avg_rating between 7 and 8 then  'hit_movie'
when avg_rating between 5 and 7 then 'one_time_watch_movie'
...
movie as m
on m.id = g.movie_id
where genre in (select genre from genre_3)
)
select *
from movie_5
where movie_rank <= 5;
...
select name as actress_name, sum(total_votes) as total_votes, count(rm.movie_id) as movie_count, avg_rating as actress_avg_rating,
